...
@interface NetworkState () <ConnectionStateWatcherDelegate>

@property (nonatomic, strong) ConnectionStateWatcher *connectionStateWatcher;
...
- (instancetype)init
{
  self = [super init];
  if (self) {
    _connectionStateWatcher = [[ConnectionStateWatcher alloc] initWithDelegate:self];
  }
  return self;
}
...
{
  ConnectionState *state = [self.connectionStateWatcher currentState];
...
- (NSDictionary *)currentDictionaryFromUpdateState:(ConnectionState *)state withInterface:(nullable NSString *)requestedInterface
{
...
- (NSMutableDictionary *)detailsFromInterface:(nonnull NSString *)requestedInterface withState:(ConnectionState *)state
{
